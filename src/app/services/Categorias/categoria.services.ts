...
    return this.http.get<Categoria[]>(this.baseServerUrl+ environment.listarCategorias);
  }

  // Crear una nueva categoría
  // POST: /api/Categorias
  registrarCategoria(categoria: Categoria): Observable<Categoria> {
    return this.http.post<Categoria>(this.baseServerUrl+ environment.crearCategorias,categoria);
  }

  // Actualizar categoría existente
  // PUT: /api/Categorias/{id}
  actualizarCategoria(idCategoria: number, categoria: Categoria): Observable<any> {
    const url = `${this.baseServerUrl+ environment.actualizarCategorias}/${idCategoria}`;
...
    const url = `${this.baseServerUrl+ environment.obtenerCategoriaPorId}/${idCategoria}`;
...
    const id = ids[0];
    const url = `${this.baseServerUrl+ environment.eliminarCategorias}/${id}`;
